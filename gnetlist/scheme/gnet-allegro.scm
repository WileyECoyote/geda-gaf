...
(define (allegro:output-netlist package)
         ...
         (newline)
         (display "CLASS ")
         (display (get-package-attribute package "class" ))
         (newline)
         (display "PINCOUNT ")
         (display (get-package-attribute package "pins" ))
         (newline)
         (let ((altfoot (get-package-attribute package "alt_foot")))
...
(define allegro:components
   (lambda (packages)
      (if (not (null? packages))
         (begin
            (let ((footprint (get-package-attribute (car packages)
                                                           "footprint"))
                  (package (car packages)))
               (if (not (string=? footprint "unknown"))
                  (display footprint))
               (display "! ")
               (display (get-package-attribute package "device"))
               (display "! ")
...
(define (allegro output-filename)
  (begin
    ...
    (display "$PACKAGES\n")
    (allegro:components netlist:packages)
    (display "$NETS\n")
    (allegro:write-net netlist:all-unique-nets)
    (display "$END\n")
    (if (not (stdout? output-filename))
       (close-output-port (current-output-port)))
    (allegro:write-device-files netlist:packages '() (stdout? output-filename))))
