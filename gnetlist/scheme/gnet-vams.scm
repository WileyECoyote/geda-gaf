  ...
  (lambda (output-filename)
    (let* (
           ...
           (architecture (vams:change-all-whitespaces-to-underlines
                          (cond
                           ((string=?
                             (get-toplevel-attribute "architecture")
           ...
           (entity (vams:change-all-whitespaces-to-underlines
                    (cond ((string=?
                            (get-toplevel-attribute "entity")
                            "not found")
                           "default_entity")
                          (else (get-toplevel-attribute "entity")))))
 ...
 (lambda (uref generic-list)
   (if (not (null? generic-list))
       (let ((attrib (car generic-list))
             (value (get-package-attribute uref (car generic-list))))
         (begin
  ...
  (lambda (ls uref)
    (begin
      (if (null? ls)
          '()
          (append
           (if (equal? (string-ref (get-package-attribute uref (car ls)) 0) #\?)
               '()
               (list (car ls)))
      ...
      (if (equal? connlist '())
          ...
          (if (= (length connlist) 1)
              (if (equal? attribute 'port_mode)
                  (if (equal? (get-attribute-by-pinnumber (car (car connlist))
                                                          (car (cdr (car connlist)))
                                                          attribute)
                              'out)
                      #t
                      #f)
                  (append (get-attribute-by-pinnumber (car (car connlist))
                                                      (car (cdr (car connlist)))
                                                      attribute)))
              (if (equal? attribute 'port_mode)
                  (if (equal? (get-attribute-by-pinnumber (car (car connlist))
...
(define vams:all-pins-nets
  (lambda (uref pins)
    (if (null? pins)
        '()
        (append (list (car (get-nets uref (car pins))))
