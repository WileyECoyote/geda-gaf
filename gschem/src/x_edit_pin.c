...
/*!* \section Pin-Properties-Dialog ( Pin Properties Editing-Dialogs ) **/
...
static WidgetStringData DialogStrings[] = {
  ...
  {"SetPinTypeSwitch",    "      Type:",     "Enable to set all selected attributes to the prescribed type"},
  {"SetAttributesSwitch", "Attributes:",     "Enable to set all selected attributes to the prescribed value"},
...
static GtkWidget *create_menu_pin_attributes ( void )
{
  ...
  for (i = 0; i < sizeof (types) / sizeof (struct pin_attribute); i++) {
    ...
    gtk_object_set_data (GTK_OBJECT(menuitem), WIDGET(PinAttributes),
...
static GtkWidget *create_menu_pin_type ( void )
{
  ...
  for (i = 0; i < sizeof (types) / sizeof (struct pin_type); i++) {
    ...
    group = gtk_radio_menu_item_group (GTK_RADIO_MENU_ITEM (menuitem));
    gtk_menu_append (GTK_MENU (menu), menuitem);
    gtk_object_set_data (GTK_OBJECT(menuitem), WIDGET(PinType),
...
x_dialog_edit_pin_type_ok(GtkWidget *Dialog, pin_type_data *pin_data)
{
  ...
  pin_objects = o_select_get_list_selected_objects(w_current, OBJ_PIN);
  ...
  if (num_selected == 1) {
    ...
    set_type       = GET_SWITCH_STATE(pin_data->set_type);
...
static void xd_edit_pin_set_sensitivity(GschemDialog *Dialog)
{
  ...
  if ( num_selected == 0 ) {  /* No pins selected so disable everything! */
    ...
    g_object_set (pin_data->set_type,       "has-tooltip", FALSE, NULL);
    ...
    gtk_widget_set_sensitive (pin_data->pin_type,        FALSE);
    gtk_widget_set_sensitive (pin_data->pin_attribute,   FALSE);
    gtk_widget_set_sensitive (pin_data->number_spin,     FALSE);
    gtk_widget_set_sensitive (pin_data->sequence_spin,   FALSE);
    gtk_widget_set_sensitive (pin_data->label_entry,     FALSE);

    /* Disable all of the Tooltips for input widgets */
    g_object_set (pin_data->pin_type,      "has-tooltip", FALSE, NULL);
    g_object_set (pin_data->pin_attribute, "has-tooltip", FALSE, NULL);
    ...
    gtk_widget_set_sensitive (pin_data->pin_type,        TRUE);
    ...
    g_object_set (pin_data->pin_type,      "has-tooltip", TRUE, NULL);
    ...
    g_object_set (pin_data->set_type,       "has-tooltip", TRUE, NULL);
    ...
    state = GET_SWITCH_STATE (pin_data->set_type);
    gtk_widget_set_sensitive (pin_data->pin_type,          state);
    g_object_set (pin_data->pin_type,       "has-tooltip", state, NULL);
...
GtkWidget *x_dialog_pin_type_create_dialog(GSCHEM_TOPLEVEL *w_current)
{
  ...
  GtkWidget *SetPinTypeSwitch    = NULL;
  ...
  type_combo_tip     = _TOOLTIP(PinType);
  attrib_combo_tip   = _TOOLTIP(PinAttributes);
  ...
  type_label = GEDA_AVM_LABEL_NEW (_LABEL(PinType), 0, 0);
  gtk_table_attach(GTK_TABLE(table), type_label, 0,1,0,1, GTK_FILL,0,0,0);

  attrib_label = GEDA_AVM_LABEL_NEW (_LABEL(PinAttributes), 0, 0);
  ...
  GSCHEM_SWITCH((GTK_WIDGET(ThisDialog)), table, SetPinType,    1, 2, FALSE)
  GSCHEM_SWITCH((GTK_WIDGET(ThisDialog)), table, SetAttributes, 2, 2, FALSE)
  ...
  GEDA_CALLBACK_SWITCH (SetPinType,    xd_edit_pin_switch_toggled, ThisDialog)
  ...
  pin_data->set_type       = SetPinTypeSwitch;
