...
/*! \defgroup Coordinates-Drag-N-Drop-Destination
 *  @{ \par This sub-group contains routines to handle signals receivable by
 *          the Coordinates-Dialog as the destination
*/
          ...
          case ENDDNDSTR:
            ...
            x_dnd_receive_string(w_current, x, y,
                                (char *) object_buffer[DND_BUFFER]->data,
                                 DROPPED_ON_COORD);
            g_free(object_buffer[DND_BUFFER]->data);
...
void x_dialog_coord_dialog (GschemToplevel *w_current, int x, int y)
{
  ...
  if (!ThisDialog) {
    ...
    g_object_set (frame, "visible", TRUE, NULL);
/*
    world_events = gtk_event_box_new();
    gtk_container_add (GTK_CONTAINER(frame), world_events);
    g_object_set (world_events, "visible", TRUE, NULL);
*/
