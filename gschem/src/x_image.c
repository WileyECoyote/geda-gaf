 ...
 * 
 *  \note.1 The returned value must be freed with g_free
    ...
    bool use_print_map, bool invert_color_bw )
{
    ...
    if (strcmp(filetype, "pdf") == 0)
      ...
      if (pixbuf != NULL) {
        if (!gdk_pixbuf_save(pixbuf, filename, filetype, &err, NULL)) {
          /* Log the error */
          s_log_message(_("x_image_lowlevel: Unable to write %s file %s. %s\n"), filetype, filename, err->message);
  ...
  /* need to do this every time you change width / height */
  set_window(toplevel, toplevel->page_current, save_page_left,
                                               save_page_right,
                                               save_page_top,
                                               save_page_bottom);
...
/*! \defgroup X-Image-Switch-Callback Functions */
