 ...
 *
 * GedaCompletion has been marked as deprecated, since this API is rarely
 ...
 *        an item in the #GedaCompletion, or %NULL if strings are going to
 *        be used as the #GedaCompletion items.
 *
 * Creates a new #GedaCompletion.
 *
...
/**
 * geda_completion_add_items:
 * @cmp: the #GedaCompletion.
 ...
 **/
void 
geda_completion_add_items (GedaCompletion* cmp,
			   GList*          items)
{
  GList* it;
...
/**
 * geda_completion_remove_items:
 * @cmp: the #GedaCompletion.
 * @items: (transfer none): the items to remove.
 *
 * Removes items from a #GedaCompletion. The items are not freed, so if the memory
 ...
 **/
void 
geda_completion_remove_items (GedaCompletion* cmp,
			   GList*	items)
{
  GList* it;
...
/**
 * geda_completion_clear_items:
 * @cmp: the #GedaCompletion.
 *
 * Removes all items from the #GedaCompletion. The items are not freed, so if the
...
static void   
completion_check_cache (GedaCompletion* cmp,
			char**	     new_prefix)
{
  register GList* list;
  register gsize len;  
 ...
 * In contrast to geda_completion_complete(), this function returns the largest common
 * prefix that is a valid UTF-8 string, omitting a possible common partial 
 * character.
 *
 * You should use this function instead of geda_completion_complete() if your 
...
/**
 * geda_completion_free:
 * @cmp: the #GedaCompletion.
 *
 * Frees all memory used by the #GedaCompletion. The items are not freed, so if
...
/**
 * geda_completion_set_compare:
 * @cmp: a #GedaCompletion.
