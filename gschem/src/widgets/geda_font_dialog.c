...
geda_font_dialog_select_best_size (GedaFontDialog *dialog)
{
...

  {
    ...
    size = g_strdup_printf ("%d", dialog->font_size ); //dialog->font_size);
...
static void callback_select_size (GtkTreeSelection *selection, gpointer data)
{
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
    ...
    size = g_strdup_printf ("%d", new_size ); //dialog->font_size);
    /* Changing the entry triggers an update to the preview entry*/
    gtk_entry_set_text (GTK_ENTRY (dialog->size_entry), size);
    g_free ( size );
...
callback_select_family (GtkTreeSelection *selection, gpointer data)
{
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter))
  {
    ...
    if ( family != dialog->family )
    {
      ...
      font_name = g_strdup_printf ("%s %d",family_name, dialog->font_size);
...
char *geda_font_dialog_get_font_name (GedaFontDialog *dialog)
{
  ...
  //g_object_get (dialog, "font-name", &fontname, NULL);
 ...
 *
 * \param obj Pointer to the dialog object.
 * \param int The integer value of the new font size.
 *
 */
void
...
/**
 * geda_font_dialog_get_preview_text:
 * @dialog: a #GedaFontDialog
 *
...
/**
 * geda_font_dialog_set_preview_text:
 * @dialog: a #GedaFontDialog
...
/**
 * geda_font_dialog_set_show_preview_entry:
 * @dialog: a #GtkFontChooser
