...
/*!****** \section Edit-Text-Dialog ( Edit Text Editing-Dialogs ) *******/
                                                 ...
                                                 OBJECT *object)
{
  ...
  if (object != NULL && object->type == OBJ_TEXT) {
    ...
    { /* Text Font Size */
      ...
      if ( GSCHEM_IS_FONT_BUTTON(widget)) {
        if (match_size) {
          gtk_widget_set_can_default (widget, TRUE);
          gschem_font_button_set_size ((GschemFontButton*)widget, text_size);
        }
        else {
...
void x_dialog_edit_text_ok(GschemToplevel *w_current, OBJECT *object)
{
  ...
  { /* Text Size */
    ...
    if ( has_value ) {
      fontname = gschem_font_button_get_font_name(GSCHEM_FONT_BUTTON (widget));
      text_size = gschem_font_button_get_size ((GschemFontButton*)widget);
    }
    else
      text_size = -1;
  }
...
void x_dialog_edit_text (GschemToplevel *w_current, OBJECT *text_object)
{
  ...
  if (!ThisDialog) {
    ...
    font_button = gschem_font_button_new_with_font (font_name);
