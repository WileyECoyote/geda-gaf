...
typedef struct {
  GedaObject *obj;     /* object that creates the pin; for net attributes it is the complex object */
  char *net;           /* non-NULL when the pin is auto-connected to a net */
} gschem_patch_pin_t;


int     gschem_patch_state_init    (gschem_patch_state_t *st, const char *fn);
int     gschem_patch_state_build   (gschem_patch_state_t *st, GedaObject *o);
