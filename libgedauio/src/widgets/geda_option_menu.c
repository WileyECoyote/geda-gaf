...
geda_option_menu_size_allocate (GtkWidget *widget, GtkAllocation *allocation)
{
  ...
  if (child && gtk_widget_get_visible (child)) {
    ...
    child_allocation.y = allocation->y + border_width + ythickness + props.focus_width + props.focus_pad + CHILD_TOP_SPACING;
...
geda_option_menu_paint (GtkWidget *widget, GdkRectangle *area)
{
  ...
  if (gtk_widget_has_focus (widget)) {
    ...
    if (props.interior_focus) {
      ...
      ypad = style->ythickness + props.focus_pad;
    ...
    gtk_paint_focus (style, widget->window, state,
                     area, widget, "button",
                     button_area.x,
                     button_area.y,
                     button_area.width,
