 ...
 * @Title: GedaComboBox
 * @See_also: #GedaComboBoxText, #GtkTreeModel, #GtkCellRenderer
 *
...
#define GEDA_COMBO_BOX_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), GEDA_TYPE_COMBO_BOX, GedaComboBoxPrivate))

struct _GedaComboBoxPrivate
{
  GtkTreeModel *model;
...
static void     geda_combo_box_menu_position_below  (GtkMenu          *menu,
                                                     ...
                                                     int              *push_in,
                                                     void             *user_data);
static void     geda_combo_box_menu_position_over   (GtkMenu          *menu,
                                                     int              *x,
                                                     int              *y,
                                                     int              *push_in,
                                                     void             *user_data);
static void     geda_combo_box_menu_position        (GtkMenu          *menu,
                                                     int              *x,
                                                     int              *y,
                                                     int              *push_in,
                                                     void             *user_data);
...
static void     geda_combo_box_menu_rows_reordered  (GtkTreeModel      *model,
                                                     ...
                                                     int               *new_order,
                                                     void              *user_data);
static void     geda_combo_box_menu_row_changed     (GtkTreeModel      *model,
                                                     GtkTreePath       *path,
                                                     GtkTreeIter       *iter,
                                                     void              *data);
...
geda_combo_box_class_init (GedaComboBoxClass *class)
{
 ...
 params = g_param_spec_boolean ("appear-as-list",
                              _("Appear as list"),
                              _("Whether dropdowns should look like lists rather than menus"),
                                 FALSE,
                                 G_PARAM_READWRITE);
...
static void
geda_combo_box_cell_layout_init (GtkCellLayoutIface *iface)
{
...
static void
geda_combo_box_cell_editable_init (GtkCellEditableIface *iface)
{
...
{
  GedaComboBoxPrivate *priv = GEDA_COMBO_BOX_GET_PRIVATE (combo_box);
...
static void
geda_combo_box_button_state_changed (GtkWidget    *widget,
				    GtkStateType  previous,
				    void *      data)
{
...
static void
geda_combo_box_check_appearance (GedaComboBox *combo_box)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
...
geda_combo_box_get_cell_info (GedaComboBox     *combo_box,
                             GtkCellRenderer *cell)
{
  GSList *i;
...
static void
geda_combo_box_menu_position_below (GtkMenu  *menu,
                                    int      *x,
                                    int      *y,
                                    int      *push_in,
                                    void     *user_data)
...
static void
geda_combo_box_menu_position_over (GtkMenu  *menu,
                                   int      *x,
                                   int      *y,
                                   bool     *push_in,
                                   void     *user_data)
...
static void
geda_combo_box_menu_position (GtkMenu  *menu,
                              int      *x,
                              int      *y,
                              int      *push_in,
                              void     *user_data)
...
static void
geda_combo_box_list_position (GedaComboBox *combo_box,
                              int          *x,
                              int          *y,
                              int          *width,
                              int          *height)
...
static bool
tree_column_row_is_sensitive (GedaComboBox *combo_box,
			      GtkTreeIter *iter)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
  GList *cells, *list;
...
static void
update_menu_sensitivity (GedaComboBox *combo_box,
			 GtkWidget   *menu)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
...
static void
geda_combo_box_menu_popup (GedaComboBox *combo_box,
                          unsigned int  button,
                          unsigned int  activate_time)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
...
/**
 * geda_combo_box_popup:
 * @combo_box: a #GedaComboBox
 *
 ...
 */
void
geda_combo_box_popup (GedaComboBox *combo_box)
{
...
static bool
geda_combo_box_real_popdown (GedaComboBox *combo_box)
{
 ...
 */
void
geda_combo_box_popdown (GedaComboBox *combo_box)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
...
static void
geda_combo_box_unset_model (GedaComboBox *combo_box)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
...
static void
geda_combo_box_forall (GtkContainer *container,
                      bool      include_internals,
                      GtkCallback   callback,
                      void *      callback_data)
{
...
static bool
geda_combo_box_expose_event (GtkWidget      *widget,
                            GdkEventExpose *event)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (widget);
  GedaComboBoxPrivate *priv = combo_box->priv;
			     ...
			     GtkTreeIter  *iter)
{
  ...
  geda_combo_box_sync_cells (combo_box, GTK_CELL_LAYOUT (cell_view));
...
static void
geda_combo_box_menu_destroy (GedaComboBox *combo_box)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
...
static void
geda_combo_box_menu_item_activate (GtkWidget *item,
                                  void    *user_data)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (user_data);
  GtkWidget *cell_view;
...
static void
geda_combo_box_update_sensitivity (GedaComboBox *combo_box)
{
  GtkTreeIter iter;
				 ...
				 void           *user_data)
{
  ...
  if (priv->tree_view)
    ...
    geda_combo_box_menu_row_changed (model, path, iter, user_data);
}

static bool
list_popup_resize_idle (void  *user_data)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (user_data);
  GedaComboBoxPrivate *priv = combo_box->priv;
  int x, y, width, height;
...
static void
geda_combo_box_list_popup_resize (GedaComboBox *combo_box)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
...
static void
geda_combo_box_menu_row_deleted (GtkTreeModel *model,
                                GtkTreePath  *path,
                                void *      user_data)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (user_data);
...
static void
geda_combo_box_menu_row_changed (GtkTreeModel *model,
                                GtkTreePath  *path,
                                GtkTreeIter  *iter,
                                void *      user_data)
{
  ...
  if (is_separator != GTK_IS_SEPARATOR_MENU_ITEM (item))
    {
      geda_combo_box_menu_row_deleted (model, path, combo_box);
      geda_combo_box_menu_row_inserted (model, path, iter, combo_box);
    }
  ...
  if (priv->wrap_width && item->parent == priv->popup_widget)
    {
      ...
      /* unattach item so geda_combo_box_relayout_item() won't spuriously
         move it */
      ...
      geda_combo_box_relayout_item (combo_box, item, iter, pitem);
    }
...
geda_combo_box_list_setup (GedaComboBox *combo_box)
{
  ...
  /* sync up */
  geda_combo_box_sync_cells (combo_box,
...
static void
geda_combo_box_list_destroy (GedaComboBox *combo_box)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
...
static bool
geda_combo_box_list_button_released (GtkWidget      *widget,
                                    GdkEventButton *event,
                                    void *        data)
{
  bool ret;
...
static bool
geda_combo_box_list_key_press (GtkWidget   *widget,
                              GdkEventKey *event,
                              void *     data)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (data);
...
static void
geda_combo_box_list_auto_scroll (GedaComboBox *combo_box,
				int         x,
				int         y)
{
...
static bool
geda_combo_box_list_scroll_timeout (GedaComboBox *combo_box)
{
  GedaComboBoxPrivate *priv = combo_box->priv;
  int x, y;
...
static bool
geda_combo_box_list_enter_notify (GtkWidget        *widget,
                                  GdkEventCrossing *event,
                                  void             *data)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (data);
...
static bool
geda_combo_box_list_select_func (GtkTreeSelection *selection,
                                 GtkTreeModel     *model,
                                 GtkTreePath      *path,
                                 bool              path_currently_selected,
                                 void             *data)
...
static void
geda_combo_box_list_row_changed (GtkTreeModel *model,
                                GtkTreePath  *path,
                                GtkTreeIter  *iter,
                                void *      data)
{
...
static void
geda_combo_box_cell_layout_pack_start (GtkCellLayout   *layout,
                                      GtkCellRenderer *cell,
                                      bool         expand)
{
...
static void
geda_combo_box_cell_layout_pack_end (GtkCellLayout   *layout,
                                    GtkCellRenderer *cell,
                                    bool         expand)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (layout);
...
static void
geda_combo_box_cell_layout_clear (GtkCellLayout *layout)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (layout);
  GedaComboBoxPrivate *priv = combo_box->priv;
  GSList *i;
 ...
 * geda_combo_box_new:
 *
 * Creates a new empty #GedaComboBox.
 *
...
/**
 * geda_combo_box_get_row_span_column:
 * @combo_box: A #GedaComboBox
 *
 ...
 */
int
geda_combo_box_get_row_span_column (GedaComboBox *combo_box)
{
  g_return_val_if_fail (GEDA_IS_COMBO_BOX (combo_box), -1);
...
/**
 * geda_combo_box_set_row_span_column:
 * @combo_box: A #GedaComboBox.
 ...
 */
void
geda_combo_box_set_row_span_column (GedaComboBox *combo_box,
                                   int         row_span)
{
  GedaComboBoxPrivate *priv;
  int col;
...
/**
 * geda_combo_box_get_column_span_column:
 * @combo_box: A #GedaComboBox
 *
...
/**
 * geda_combo_box_set_column_span_column:
 * @combo_box: A #GedaComboBox
...
/**
 * geda_combo_box_get_active_iter:
 * @combo_box: A #GedaComboBox
 ...
 */
bool
geda_combo_box_get_active_iter (GedaComboBox *combo_box,
                                GtkTreeIter  *iter)
{
  GtkTreePath *path;
  bool result;
...
/**
 * geda_combo_box_set_active_iter:
 * @combo_box: A #GedaComboBox
 ...
 */
void
geda_combo_box_set_active_iter (GedaComboBox     *combo_box,
                               GtkTreeIter     *iter)
{
  GtkTreePath *path = NULL;
  ...
  if (iter)
    path = gtk_tree_model_get_path (geda_combo_box_get_model (combo_box), iter);
 ...
 * Since: 2.4
 *
 * Deprecated: 2.24: Use #GedaComboBoxText
 */
GtkWidget *
geda_combo_box_new_text (void)
{
  GtkWidget *combo_box;
  GtkCellRenderer *cell;
  GtkListStore *store;
  ...
  combo_box = geda_combo_box_new_with_model (GTK_TREE_MODEL (store));
...
/**
 * geda_combo_box_append_text:
 * @combo_box: A #GedaComboBox constructed using geda_combo_box_new_text()
 * @text: A string
 *
...
/**
 * geda_combo_box_prepend_text:
 * @combo_box: A #GedaComboBox constructed with geda_combo_box_new_text()
 * @text: A string
 *
 ...
 */
void
geda_combo_box_remove_text (GedaComboBox *combo_box,
                           int         position)
{
  GtkTreeIter iter;
  GtkListStore *store;
 ...
 * Since: 2.6
 *
 * Deprecated: 2.24: If you used this with a #GedaComboBox constructed with
 * geda_combo_box_new_text() then you should now use #GedaComboBoxText and
 * geda_combo_box_text_get_active_text() instead. Or if you used this with a
 * #GedaComboBoxEntry then you should now use #GedaComboBox with
 * #GedaComboBox:has-entry as %TRUE and use
...
static bool
geda_combo_box_mnemonic_activate (GtkWidget *widget,
				  bool      group_cycling)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (widget);
...
static void
geda_combo_box_grab_focus (GtkWidget *widget)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (widget);
...
static void
geda_combo_box_destroy (GtkObject *object)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (object);
...
static void
geda_combo_box_dispose(GObject* object)
{
  GedaComboBox *combo_box = GEDA_COMBO_BOX (object);
  ...
  G_OBJECT_CLASS (geda_combo_box_parent_class)->dispose (object);
}
...
/**
 * geda_combo_box_get_add_tearoffs:
 * @combo_box: a #GedaComboBox
 *
 ...
 */
void
geda_combo_box_set_add_tearoffs (GedaComboBox *combo_box,
				bool     add_tearoffs)
{
  g_return_if_fail (GEDA_IS_COMBO_BOX (combo_box));
...
/**
 * geda_combo_box_get_popup_accessible:
 * @combo_box: a #GedaComboBox
 *
...
/**
 * geda_combo_box_get_row_separator_func:
 * @combo_box: a #GedaComboBox
 *
...
/**
 * geda_combo_box_set_row_separator_func:
 * @combo_box: a #GedaComboBox
...
/**
 * geda_combo_box_set_button_sensitivity:
 * @combo_box: a #GedaComboBox
...
/**
 * geda_combo_box_get_button_sensitivity:
 * @combo_box: a #GedaComboBox
 *
...
/**
 * geda_combo_box_set_entry_text_column:
 * @combo_box: A #GedaComboBox
...
/**
 * geda_combo_box_get_entry_text_column:
 * @combo_box: A #GedaComboBox.
 *
...
/**
 * geda_combo_box_set_focus_on_click:
 * @combo: a #GedaComboBox
...
/**
 * geda_combo_box_get_focus_on_click:
 * @combo: a #GedaComboBox
 *
 ...
 */
bool
geda_combo_box_get_focus_on_click (GedaComboBox *combo_box)
{
  g_return_val_if_fail (GEDA_IS_COMBO_BOX (combo_box), FALSE);
...
static bool
geda_combo_box_buildable_custom_tag_start (GtkBuildable  *buildable,
                                           GtkBuilder    *builder,
                                           GObject       *child,
                                           const char    *tagname,
                                           GMarkupParser *parser,
...
static void
geda_combo_box_buildable_custom_tag_end (GtkBuildable *buildable,
                                         GtkBuilder   *builder,
                                         GObject      *child,
                                         const char   *tagname,
                                         void        **data)
