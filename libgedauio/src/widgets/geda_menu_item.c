                                                      ...
                                                      void         *user_data)
{
  if (strcmp (tagname, "accelerator") == 0) {
    ...
    menu_shell = geda_get_widget_parent ((GtkWidget*)buildable);
    ...
    if (menu_shell) {
      ...
      {
        menu_shell = geda_get_widget_parent (attach);
      }
 ...
 * \par Function Description
 *  Sets or replaces the submenu of a menu item, or removes the current
 *  submenu when a %NULL submenu is passed. This function does not call
 ...
 * \par Function Description
 *  Sets or replaces the submenu of menu item, or removes the current
 *  submenu when \a submenu is %NULL.
 *
 * \param [in] menu_item a #GedaMenuItem
...
static int geda_menu_item_popup_timeout (void *data)
{
      ...
      (parent_is_menu && ((GedaMenu*)parent)->torn_off))
  {
    ...
    if (priv->timer_from_keypress && priv->submenu) {
      ((GedaMenuShell*)priv->submenu)->ignore_enter = TRUE;
    }
  }
