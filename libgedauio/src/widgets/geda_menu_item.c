...
geda_menu_item_class_init  (void *class, void *class_data)
{
  ...
  menu_item_signals[TOGGLE_SIZE_REQUEST] =
    g_signal_new (_("toggle-size-request"),
                  G_OBJECT_CLASS_TYPE (gobject_class),
                  G_SIGNAL_RUN_FIRST,
  ...
  menu_item_signals[TOGGLE_SIZE_ALLOCATE] =
    g_signal_new (_("toggle-size-allocate"),
                  G_OBJECT_CLASS_TYPE (gobject_class),
                  G_SIGNAL_RUN_FIRST,
   ...
   *
   * use the standard padding CSS property (through objects
                                ...
                                int       *natural_size)
{
  ...
  if (child != NULL && gtk_widget_get_visible (child)) {
    ...
    int child_min, child_nat;
    int arrow_size = 0, arrow_spacing = 0;
...
/*!
 * \brief geda_menu_item_set_label:
 *  Sets @text on the @menu_item label
 *
 * \param [in] menu_item a GedaMenuItem
 * \param [in] label     text to be used in the label
 */
void
                          ...
                          const char   *label)
{
  ...
  GEDA_MENU_ITEM_GET_CLASS (menu_item)->set_label (menu_item, label);
}
