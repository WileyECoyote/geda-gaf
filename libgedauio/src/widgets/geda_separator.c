  ...
  if (seperator->orientation == 0) { /* Horizontal */
    ...
    if (wide_separators) {
        gtk_paint_box (style, window, state, GTK_SHADOW_ETCHED_OUT,
                       ...
                       allocation->x,
                       allocation->y +
                      (allocation->height - separator_height) / 2,
                       allocation->width,
                       separator_height);
    }
    else {
    ...
    if (wide_separators) {
      gtk_paint_box (style, window, state, GTK_SHADOW_ETCHED_OUT,
                     ...
                     allocation->x +
                    (allocation->width - separator_width) / 2,
                     allocation->y,
                     separator_width,
                     allocation->height);
    }
    else {
      gtk_paint_vline (style, window, state,
                       &event->area, widget, "vseparator",
                       allocation->y,
...
static void geda_separator_instance_init(GTypeInstance *instance, void *g_class)
{
  ...
  requisition = geda_get_widget_requisition(instance);
  style       = geda_get_widget_style (instance);
...
/*!
 * \brief  Create New Vertical GedaSeparator
 * \par Function Description
 *  Creates a new #GedaSeparator with vertical orientation.
 *
 * \return a new #GedaSeparator with vertical orientation
 */
