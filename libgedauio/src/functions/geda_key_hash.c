/* geda_key_hash.c: Keymap aware matching of key bindings
 *
...
static void
key_hash_clear_keycode (void *key, void *value, void *data)
{
  GSList *keys = value;
  g_slist_free (keys);
}
...
static GHashTable *
key_hash_get_keycode_hash (GedaKeyHash *key_hash)
{
  if (!key_hash->keycode_hash) {
    ...
    hash = g_list_last (key_hash->entries_list);

    for (tmp_list = hash; tmp_list; tmp_list = tmp_list->prev) {
...
static void
key_hash_keys_changed (GdkKeymap *keymap, GedaKeyHash *key_hash)
{
...
static void
key_hash_free_entry (GedaKeyHash *key_hash, GedaKeyHashEntry *entry)
{
...
static void
key_hash_free_entry_foreach (void *value, void *data)
{
  GedaKeyHashEntry *entry    = value;
...
void geda_key_hash_remove_entry (GedaKeyHash *key_hash, void *value)
{
  ...
  if (entry_node) {
    ...
    GedaKeyHashEntry *entry = entry_node->data;
...
static bool keyval_in_group (GdkKeymap  *keymap, unsigned int keyval, int group)
{
  ...
  gdk_keymap_get_entries_for_keyval (keymap, keyval, &entry.keys, &entry.n_keys);
                              ...
                              int              group)
{
  ...
  bool group_mod_is_accel_mod = FALSE;
  ...
  geda_keyboard_translate_accel_state (key_hash->keymap,
  ...
  if (keys) {
    ...
    while (tmp_list) {
      ...
      GedaKeyHashEntry *entry = tmp_list->data;
  ...
  if (!have_exact && results) {
     ...
     */
    unsigned int oldkeyval = 0;
    GedaKeyHashEntry *keyhashentry;
                                     ...
                                     GdkModifierType modifiers)
{
  ...
  if (n_keys) {
    ...
    while (entries) {
      ...
      GedaKeyHashEntry *entry = entries->data;
