...
const char  *o_file_format_header          (void);
      ...
      GList *o_read_buffer                 (TOPLEVEL *toplevel, GList *object_list, const char *buffer,
...
/* g_rc.c */
      ...
      void g_gafrc_parse_handler  (TOPLEVEL *toplevel, const char *dummy,  ConfigParseErrorFunc handler, void *user_data);
      void g_rcname_parse_handler (TOPLEVEL *toplevel, const char *rcname, ConfigParseErrorFunc handler, void *user_data);
      void g_rcfile_parse_handler (TOPLEVEL *toplevel, const char *rcfile, ConfigParseErrorFunc handler, void *user_data);
      void g_rc_parse_handler     (TOPLEVEL *toplevel, const char *rcname, const char *rcfile,
...
/* o_basic.c */
      ...
      void       o_set_line_options                  (TOPLEVEL *toplevel, OBJECT *o_current, OBJECT_END end,
                                                      OBJECT_TYPE type, int width, int length, int space);
      bool       o_get_line_options                  (OBJECT *object, OBJECT_END *end, OBJECT_TYPE *type,
                                                      int *width, int *length, int *space);
      void       o_set_fill_options                  (TOPLEVEL *toplevel, OBJECT *o_current, OBJECT_FILLING type,
                                                      int width, int pitch1, int angle1, int pitch2, int angle2);
...
const char      *o_get_page_file_extension           (PAGE *page);
      ...
      void       o_add_change_notify                 (TOPLEVEL *toplevel, ChangeNotifyFunc pre_change_func,
                                                      ChangeNotifyFunc change_func, void *user_data);
      void       o_remove_change_notify              (TOPLEVEL *toplevel, ChangeNotifyFunc pre_change_func,
                                                      ChangeNotifyFunc change_func, void *user_data);
...
/* o_bus_basic.c */
      OBJECT    *o_bus_new                 (TOPLEVEL *toplevel, char type, int color, int x1, int y1, int x2, int y2,
...
/* o_complex_basic.c */
      ...
      OBJECT    *o_complex_new                  (TOPLEVEL *toplevel, char type, int color,
                                                 int x, int y, int angle, int mirror, const CLibSymbol *clib_sym,
                                                 const char *basename, int selectable);
      OBJECT    *o_complex_new_embedded         (TOPLEVEL *toplevel, char type, int color,
                                                 int x, int y, int angle, int mirror, const char *basename, int selectable);
...
/* o_line_basic.c */
      ...
      void       o_line_rotate_world       (TOPLEVEL *toplevel, int world_centerx, int world_centery,
                                            int angle, OBJECT *object);
...
/* o_picture.c */
      OBJECT    *o_picture_new             (TOPLEVEL *toplevel, const char *file_content, gsize file_length,
                                            const char *filename, char type,
...
const char      *o_picture_get_data        (TOPLEVEL *toplevel, OBJECT *object, size_t *len);
      bool       o_picture_set_from_buffer (TOPLEVEL *toplevel, OBJECT *object, const char *filename, const char *data, size_t len, GError  **error);
      bool       o_picture_set_from_file       (TOPLEVEL *toplevel, OBJECT *object,
                                                const char *filename, GError **error);
const char      *o_picture_get_filename        (TOPLEVEL *toplevel, OBJECT *object);
      GdkPixbuf *o_picture_get_fallback_pixbuf (TOPLEVEL *toplevel) G_GNUC_WARN_UNUSED_RESULT;

/* o_pin_basic.c */
      OBJECT    *o_pin_new             (TOPLEVEL *toplevel, char type, int color, int x1, int y1,
                                        int x2, int y2, PIN_TYPE pin_type, int whichend);
...
const char       *s_path_sys_data               (void);
const char       *s_path_sys_doc                (void);
const char       *s_path_sys_config             (void);
const char       *s_path_user_config            (void);
...
/* s_hierarchy.c */
      PAGE  *s_hierarchy_down_schematic_single  (TOPLEVEL *toplevel, const char *filename, PAGE *parent,
