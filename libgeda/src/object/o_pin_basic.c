...
o_pin_read (const char buf[], unsigned int release_ver, unsigned int fileformat_ver, GError **err)
{
  ...
  if (whichend == -1) {
    u_log_message (_("Found a pin which did not have the whichend field set.\n"
    "Verify and correct manually.\n"));
  }
  else if (whichend < -1 || whichend > 1) {
    u_log_message (_("Found an invalid whichend on a pin (reseting to zero): %d\n"),
                   whichend);
    whichend = 0;
  }
...
o_pin_rotate_world(int world_centerx, int world_centery, int angle, Object *object)
{
  ...
  rotate_point_90(object->line->x[0], object->line->y[0], angle, &newx, &newy);

  object->line->x[0] = newx;
  object->line->y[0] = newy;

  rotate_point_90(object->line->x[1], object->line->y[1], angle, &newx, &newy);
 ...
 *
 *  \note geda_pin_lookup_estring returns NULL if the code was
 *  not valid but the function do not report as an error and this
...
o_pin_create_seq_attrib(GedaToplevel *toplevel, Object *object, int sequence, int x, int y)
{
  ...
  text = g_strdup_printf("pinseq=%d", value);
...
/*! \brief Update a Pin Object Property with Read Attribute Object
 *
 *  \par Function Description
 ...
 *  oject to the Pin Object to set internal values.
 *
