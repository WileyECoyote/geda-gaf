...
{
  const char *path = geda_file_path_sys_config();
...
{
  /* geda_file_path_sys_data() returns a raw environment string, so assume
   * it's in the current locale's encoding. */
  const char *path = geda_file_path_sys_data();
  SCM dir = scm_from_locale_string (path);
  return scm_list_1 (dir);
}
...
{
  const char *path = geda_file_path_user_config();
  SCM dir = scm_from_locale_string (path);
  return scm_list_1 (dir);
}
