 ...
 */
SCM_DEFINE (filename, "%page-filename", 1, 0, 0,
            ...
            (SCM page_s, SCM obj_s), "Remove an object from a page.")
{
  ...
  /* Ensure that the arguments have the correct types. */
  SCM_ASSERT (EDASCM_PAGEP (page_s), page_s, SCM_ARG1, s_page_remove_x);
  SCM_ASSERT (EDASCM_OBJECTP (obj_s), obj_s, SCM_ARG2, s_page_remove_x);
  ...
  if ((curr_page != NULL && curr_page != page) || (obj->parent_object != NULL)) {
...
init_module_geda_core_page ()
{
  ...
  scm_c_export (s_active_pages, s_new_page, s_close_page_x,
                s_filename, s_set_filename_x, s_page_contents,
