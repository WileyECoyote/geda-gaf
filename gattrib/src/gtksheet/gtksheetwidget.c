...
gtk_sheet_class_init (GtkSheetClass * klass)
{
  ...
  gtk_container_class_install_child_property (container_class, CHILD_PROP_Y, pspec);
  ...
  gtk_container_class_install_child_property (container_class, CHILD_PROP_ROW, pspec);
  ...
  pspec = g_param_spec_int ("column",
                            ...
                            0, G_MAXINT, 0,
                            G_PARAM_READWRITE);
  gtk_container_class_install_child_property (container_class, CHILD_PROP_COLUMN, pspec);

  pspec = g_param_spec_boolean ("attached-to-cell",
                                "Whether child is attached",
                                "Whether the child's position is kept attached to the cell indicated by 'row' and 'column'",
                                TRUE,
                                G_PARAM_READWRITE);

  gtk_container_class_install_child_property (container_class, CHILD_PROP_ATTACHED_TO_CELL, pspec);

  pspec = g_param_spec_boolean ("floating",
                                "Resize child with the cell",
                                "Whether the child size is kept within the area of the cell indicated by 'row' and 'column'",
                                FALSE,
                                G_PARAM_READWRITE);
  gtk_container_class_install_child_property (container_class, CHILD_PROP_FLOATING, pspec);

  pspec = g_param_spec_flags ("x-options",
                              "Horizontal options",
                              "Options specifying the horizontal behaviour of the child",
                              GTK_TYPE_ATTACH_OPTIONS, GTK_EXPAND | GTK_FILL,
                              G_PARAM_READWRITE);
  gtk_container_class_install_child_property (container_class, CHILD_PROP_X_OPTIONS, pspec);

  pspec = g_param_spec_flags ("y-options",
                              "Vertical options",
                              "Options specifying the vertical behaviour of the child",
                              GTK_TYPE_ATTACH_OPTIONS, GTK_EXPAND | GTK_FILL,
                              G_PARAM_READWRITE);

  gtk_container_class_install_child_property (container_class, CHILD_PROP_Y_OPTIONS, pspec);

  pspec = g_param_spec_uint ("x-padding",
                             "Horizontal padding",
                             "Extra space to put between the child and the left and right cell borders, in pixels",
                             0, 65535, 0,
                             G_PARAM_READWRITE);
  gtk_container_class_install_child_property (container_class, CHILD_PROP_X_PADDING, pspec);

  pspec = g_param_spec_uint ("y-padding",
                             "Vertical padding",
                             "Extra space to put between the child and the upper and lower cell borders, in pixels",
                             0, 65535, 0,
                             G_PARAM_READWRITE);
  gtk_container_class_install_child_property (container_class, CHILD_PROP_Y_PADDING, pspec);
}
...
gtk_sheet_show_active_cell(GtkSheet *sheet)
{
 ...
 g_object_set ( GTK_WIDGET(sheet->sheet_entry), "visible", TRUE, NULL);
